function mitsubaScene = mMitsubaImportMexximp(mexximpScene, varargin)
%% Convert a mexximp scene struct to an mMitsuba scene object.
%
% mitsubaScene = mMitsubaImportMexximp(mexximpScene) converts the given
...
% including:
%   - mMitsubaImportMexximpCamera()
%   - mMitsubaImportMexximpLight()
%   - mMitsubaImportMexximpMaterial()
%   - mMitsubaImportMexximpMesh()
%   - mMitsubaImportMexximpNode()
...
%
% mitsubaScene = mMitsubaImportMexximp(mexximpScene, varargin)
%
...
for cc = cameraInds
    mitsubaNode = mMitsubaImportMexximpCamera(mexximpScene, elements(cc), varargin{:});
...
for mm = 1:nMaterials
    mitsubaNode = mMitsubaImportMexximpMaterial(mexximpScene, elements(materialInds(mm)), varargin{:});
    mitsubaScene.append(mitsubaNode);
...
for ll = lightInds
    mitsubaNode = mMitsubaImportMexximpLight(mexximpScene, elements(ll), varargin{:});
    mitsubaScene.append(mitsubaNode);
end
...
for mm = 1:nMeshes
    [~, meshFiles{mm}] = mMitsubaImportMexximpMesh(mexximpScene, elements(meshInds(mm)), varargin{:});
end

% PLY shape element for each node.
nodeInds = find(strcmp('nodes', elementTypes));
for nn = nodeInds
    mitsubaNodes = mMitsubaImportMexximpNode(mexximpScene, elements(nn), meshFiles, materialIds, varargin{:});
